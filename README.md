...
- **A first-cut Logic Editor** (the Events tab): pick an element on the
  ...
  uses, and actions can play **declarative animations (duration, easing, direction)** (NUI-SCHEMA §8.3) via the `Nyrqis.Animation.Play` system action — a behavior fires `WHEN StartMenu.Opened DO animation StartMenu.Open` as a timed, eased property transition, optionally along a multi-point **keyframe** curve (strictly increasing offsets in [0, 1], each with a value). The **animation timeline editor** (Animations tab) edits each animation's timing, easing, and keyframes inline: add/remove keyframes, edit offset/value, with an inline keyframe list per animation.
